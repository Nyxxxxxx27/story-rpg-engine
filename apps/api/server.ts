...
import { createStoryRequestSchema, createTurnRequestSchema, outlineDraftSchema, storyStageSchema, storyWorldConfigSchema } from '../../packages/contracts/index.ts';

const owner = 'local-player';
const idParams = z.object({ id: z.string().uuid() });
const storyTurnParams = z.object({ id: z.string().uuid(), turnId: z.string().uuid() });

export async function createStoryServer(options: { database?: Database; startWorker?: boolean; logger?: boolean } = {}) {
  const database = options.database ?? await connectDatabase(); const ownsDatabase = !options.database;
  ...
  app.setErrorHandler((error, _request, reply) => {
    ...
    const status = failure.message.includes('NOT_FOUND') ? 404 : failure.message === 'UNAUTHORIZED' ? 403 : failure.name === 'ZodError' ? 400 : 500;
    reply.status(status).send({ error: failure.message, code: status === 500 ? 'INTERNAL_ERROR' : 'INVALID_REQUEST' });
  });
  app.get('/api/v2/health', async () => ({ ok: true, version: 'story-v1', provider: 'explicit-per-story' }));
  ...
  app.post('/api/v2/stories/:id/turns', async request => {
    ...
    const turn = await store.enqueueTurn(storyId, body.input, body.source, body.idempotencyKey); await store.flushOutbox(); return turn;
  });
  app.post('/api/v2/stories/:id/choices/resolve', async request => { const storyId = idParams.parse(request.params).id; await store.state(storyId, owner); const { choice } = z.object({ choice: z.string().trim().min(1).max(500) }).parse(request.body); return store.resolveChoice(storyId, choice); });
  app.get('/api/v2/stories/:id/turns/:turnId', async request => {
    const params = storyTurnParams.parse(request.params); await store.state(params.id, owner); return store.turn(params.turnId);
  });
  ...
  app.get('/api/v2/stories/:id/stage-proposal', async request => { const storyId = idParams.parse(request.params).id; await store.state(storyId, owner); return { proposal: await store.pendingProposal(storyId) }; });
  ...
  app.get('/api/v2/stories/:id/events', async (request, reply) => {
    ...
    await emit(); const timer = setInterval(() => void emit(), 500); request.raw.on('close', () => clearInterval(timer));
  });
