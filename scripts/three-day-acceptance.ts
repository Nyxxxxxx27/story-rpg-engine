...
const request = async (path: string, init?: RequestInit) => new Promise<any>((resolveRequest, rejectRequest) => {
  ...
  const client = httpRequest(`http://127.0.0.1:4310/api/v2${path}`, { method: init?.method ?? 'GET', headers: requestBody ? { 'content-type': 'application/json', 'content-length': Buffer.byteLength(requestBody) } : undefined }, response => {
