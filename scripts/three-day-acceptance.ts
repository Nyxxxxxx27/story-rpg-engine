...
const root = resolve('.'); const dataDirectory = resolve('.data/acceptance-three-day'); const reportDirectory = resolve('reports/acceptance');
await rm(dataDirectory, { recursive: true, force: true }); await mkdir(dataDirectory, { recursive: true }); await mkdir(reportDirectory, { recursive: true }); process.env.STORY_DATA_DIR = dataDirectory; process.env.STORY_SHUTDOWN_DIAGNOSTICS = '1';
...
let service = await createStoryServer({ logger: false }); await service.app.listen({ host: '127.0.0.1', port: 4310 }); let vite: ViteDevServer | undefined; let browser: Browser | undefined; let page: Page | undefined; let mcp: Client | undefined; let restarted = false; let restartDurationMs: number | null = null; let stageProposals = 0; let choicePauses = 0;
const startedAt = new Date().toISOString(); const pauseReasons: string[] = []; const notes: string[] = []; const pageErrors: string[] = [];
const request = async (path: string, init?: RequestInit) => { const response = await fetch(`http://127.0.0.1:4310/api/v2${path}`, { ...init, headers: { ...(init?.body ? { 'content-type': 'application/json' } : {}) } }); const body = await response.json(); if (!response.ok) throw new Error(`${path}: ${body.error ?? response.status}`); return body; };
...
const connectMcp = async () => { const client = new Client({ name: 'three-day-acceptance', version: '1.0.0' }, { capabilities: {} }); const transport = new StdioClientTransport({ command: 'node', args: ['--import', 'tsx', 'apps/mcp/main.ts'], cwd: root, env: { ...process.env, STORY_API_URL: 'http://127.0.0.1:4310/api/v2' } as Record<string, string>, stderr: 'pipe' }); await client.connect(transport); return client; };
...
try {
  ...
  outline = { ...outline, stages: outline.stages.map((stage: any, index: number) => index === 0 ? { ...stage, objective: `在故事首日完成：${stage.objective}`, boundaries: [...stage.boundaries, '第一阶段结束时，是否公开已核实线索或继续保密由玩家决定'] } : stage) }; await request(`/stories/${storyId}/outline`, { method: 'PUT', body: JSON.stringify(outline) });
  vite = await createVite({ configFile: resolve('vite.config.ts') }); await vite.listen(); browser = await chromium.launch({ headless: true }); page = await browser.newPage({ viewport: { width: 1440, height: 1000 } }); page.on('pageerror', error => pageErrors.push(error.message)); await page.goto('http://127.0.0.1:4173');
  ...
  if (opening.status === 'waiting_player') { choicePauses += 1; pauseReasons.push(opening.waitingReason ?? 'opening choice'); console.log(`[acceptance] resolving opening choice: ${opening.waitingReason}`); await post(`/stories/${storyId}/choices/resolve`, { choice: '选择保守且可撤回的路线：保护同伴与现有证据，优先核验旁证。' }); }
  ...
  while (Date.now() < deadline) {
    ...
    if (!restarted && state.clock - autoplayStart >= 2160) {
      console.log('[acceptance] restarting API, worker, embedded PostgreSQL socket, and Codex Provider'); const restartStarted = Date.now(); await service.app.close(); service = await createStoryServer({ logger: false }); await service.app.listen({ host: '127.0.0.1', port: 4310 }); restartDurationMs = Date.now() - restartStarted; restarted = true; notes.push(`在至少 2160 分钟处用 ${restartDurationMs} ms 完成服务与 Provider 重启，恢复后无重复场景。`); await new Promise(resolvePromise => setTimeout(resolvePromise, 1000)); state = await request(`/stories/${storyId}`);
    }
    if (session?.status === 'completed') break;
    if (session?.status === 'paused') {
      pauseReasons.push(session.pauseReason ?? 'unknown'); const proposal = (await request(`/stories/${storyId}/stage-proposal`)).proposal;
      if (proposal) { stageProposals += 1; console.log(`[acceptance] accepting stage proposal: ${proposal.stage.title}`); await post(`/stories/${storyId}/stage-proposal/${proposal.id}/review`, { decision: 'accept' }); }
      else {
        choicePauses += 1; console.log(`[acceptance] resolving conservative choice: ${session.pauseReason}`); await post(`/stories/${storyId}/choices/resolve`, { choice: '选择保守方案：先保护核心角色与现有证据，只进行可撤回的下一步。' }); await post(`/stories/${storyId}/autoplay/resume`);
      }
    }
  ...
  const allFacts = await service.database.pool.query('SELECT id,seq,source_turn_id,payload FROM facts WHERE story_id=$1 ORDER BY seq', [storyId]); const allScenes = await service.database.pool.query('SELECT data FROM scenes WHERE story_id=$1 ORDER BY seq', [storyId]); const hashes = await service.store.stateHashes(storyId);
  ...
  const assertions = {
    exact4320: state.clock === autoplayStart + 4320, atLeast10Scenes: allScenes.rows.length >= 10, stageProposalReviewed: stageProposals >= 1,
    ...
    choicePauseObserved: choicePauses >= 1,
  };
